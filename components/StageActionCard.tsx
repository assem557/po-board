...
import { Upload, CheckSquare, Square, ArrowRight, Lock, AlertCircle, Sparkles } from 'lucide-react'
...
export default function StageActionCard({ poId, currentStage, isExistingDealer, onAdvanced }: Props) {
  ...
  const accent = TEAM_ACCENT[stage.team] ?? '#475569'
        ...
        {effectiveCheckboxes.length > 0 && (
          <div className="space-y-2">
            {effectiveCheckboxes.map(cb => (
              <button
                key={cb}
                onClick={() => isMyStage && setCheckedBoxes(p => ({ ...p, [cb]: !p[cb] }))}
                disabled={!isMyStage}
              ...
              >
                <div
                  ...
                  style={{
                    background: checkedBoxes[cb] ? accent : 'transparent',
                    border: `1.5px solid ${checkedBoxes[cb] ? accent : 'var(--border)'}`,
                  }}
                >
                  {checkedBoxes[cb] && <svg width="10" height="8" viewBox="0 0 10 8" fill="none">
